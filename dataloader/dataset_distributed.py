...
class SemKITTI_pt(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print("###", pixel_coordinates1.shape)
        # print("$$$", mask1.shape)
        # ### (116471, 2)
        # $$$ (116471,)
        ...
        if self.split in ["train", "val"]:
            ...
            data_tuple += (-1, -1)
        #print("###", sem_data.dtype) #  int64
        #print("$$$", inst_data.dtype) # uint32
        #exit()
        # gh
        fusion_tuple = (xyz_segfea, xyz_pixfea)  # gh todo not mono?

        data_tuple += (fusion_tuple,)  # (data_tuple, (fusion_tuple))
        # print("!!!", pts[:, :3].dtype) float32
        # print("%%%", pts[:,3].dtype) float32
        # print("@@@", xyz_segfea.dtype) torch.float32
        # print("###", xyz_pixfea.dtype) torch.float32
        # exit()
        return data_tuple
...
class Nuscenes_pt(data.Dataset):
    ...
    def _load_sam(self, channel, file_id):
        ...
        pixloc = torch.load(os.path.join(dst_img, channel, file_id + "_idx.pt"), map_location='cpu')
        segidx = torch.load(os.path.join(dst_seg, channel, file_id + "_idx.pt"), map_location='cpu')
    ...
    def __getitem__(self, index):
        ...
        for idx, channel in enumerate(self.CAM_CHANNELS):
            ...
            # In Nuscene, we should consider only on masked parts
            sub_pts_t = points[:, :3][mask]
            # sub_pts_ids = points_ids[mask]
            ...
            #
            # xyz_segfea = segfea[cluster_ids]  # (n_pts,3,256)
            # xyz_segscore = segscore[cluster_ids]  # (n_pts,3)
            # gh
            # assign zero values for far points from the anchor
            # xyz_segfea[dist_mask] = 0
            # xyz_segscore[dist_mask] = 0

            # valid_idx = torch.logical_not(dist_mask)
            ...
            if len(xyz_segfea[mask]) != len(segidx.T):
                breakpoint()

            imgfea = torch.zeros((256, 64, 64), dtype=torch.float32)
            imgfea[:, pix_loc[0], pix_loc[1]] = imgfea_sub.to(torch.float32)
            uvs = pixel_coord.T[mask].astype(np.int32)

            grid_tmp = torch.Tensor(uvs[None, :, None, :])  # (1, n_pts, 1, 2)

            xyz_pixfea = torch.nn.functional.grid_sample(imgfea.to(torch.float32), grid_tmp)[0, ..., 0].transpose(1,
                                                                                                                  0)  # (n_pts,256)
        ...
        # total_xyz_segscore = total_xyz_segscore / torch.clamp(count_xyz, 1.0)[:, None]
        ...
        # fusion_tuple = (total_xyz_segfea, total_xyz_pixfea, total_xyz_segscore)
...
class spherical_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print("!!!", xyz.shape) (127810, 3)
        # print("@@@", feat.shape) (127810,)
        # print("###", token)  07_000360
        # print("$$$", labels.shape) (127810,)
        # print("%%%", insts.shape) (127810, 1)
        # print("^^^", fusion_tuple[0].shape) torch.Size([127810, 3, 256])
        # print("&&&", fusion_tuple[1].shape) torch.Size([127810, 256])
        ...
        label_voxel_pair = label_voxel_pair[np.lexsort((current_grid[:, 0], current_grid[:, 1], current_grid[:, 2])), :]
            # print("""###""", label_voxel_pair.shape) : (127810, 4)
            #
            # print("""@@@""", np.copy(voxel_label).shape) : (480, 360, 32)
            #
            # print("""$@$""", label_voxel_pair.shape) : (125601, 4)


        voxel_label = nb_process_label(np.copy(voxel_label), label_voxel_pair)
        # print("###", np.copy(voxel_label).dtype) uint8
        # print("@@@", label_voxel_pair.dtype) int64
        # exit()
